        ...
        return TheDb.getPragmaVersion()
            .then((version) => {
                if (version === TheDb.version) {
                    const sql = fs.readFileSync(path.join(Settings.dbFolder, `changes_admin.db.sql`), {encoding: 'utf8'});
                    TheDb.exec(sql)
                    return Promise.resolve();
                }
                    ...
                    return new Promise<void>((resolve, reject) => {
                        const sql = fs.readFileSync(path.join(Settings.dbFolder, `changes_v${TheDb.version}.db.sql`), {encoding: 'utf8'});
                        switch (TheDb.version) {
                            case 2:
                                TheDb.exec(sql);
